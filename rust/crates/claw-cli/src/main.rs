...
impl DefaultRuntimeClient {
    fn new(
        ...
        // Build the right provider client based on the model name.
        // For Anthropic/Claw models we still honour the legacy OAuth/API-key auth;
        // for Gemini and xAI we read their respective env-var keys.
        let resolved = api::resolve_model_alias(&model);
        let client = match api::detect_provider_kind(&resolved) {
            api::ProviderKind::ClawApi => {
                let auth = resolve_cli_auth_source()?;
                ProviderClient::from_model_with_default_auth(&resolved, Some(auth))?
            }
            _ => ProviderClient::from_model(&resolved)?,
        };
        Ok(Self {
        ...
        match self.tool_registry.execute(tool_name, &value) {
            Ok(output) => {
                if self.emit_output {
                    ...
                    self.renderer
                        .stream_markdown(&markdown, &mut lock)
